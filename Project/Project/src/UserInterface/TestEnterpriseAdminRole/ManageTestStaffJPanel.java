...
import Business.Role.TestDefinerRole;
import Business.Role.TestReporterRole;
 ...
 * @author amishagupta
 */
public class ManageTestStaffJPanel extends javax.swing.JPanel {
    ...
    public ManageTestStaffJPanel(JPanel userProcessContainer,OrganizationDirectory organizationDir,Enterprise ent) {
        initComponents();
...
//        organizationNurseJTable1.getTableHeader().setDefaultRenderer(new HeaderColor());
    ...
    private void populateDoctorTable(){
        ...
        for (TestDefiner onl : org.getTestDefinerDirectory().getTestDefinerList()){
         ...
         for (TestReporter onl : org.getTestReporterDirectory().getTestReporterList()){
            Object[] row = new Object[5];
            row[0] = onl;         
            row[1] = onl.getContactNumber();
            row[2] = onl.getZipcode();
...
//    }
    ...
    private void initComponents() {
        ...
        add(jPanel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(50, 270, 760, 350));
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if(orga.getType().getValue().equalsIgnoreCase("Test Definer")){
            TestDefiner testDefiner = new TestDefiner(name, contact, zipcode, city, gender, username, password, dateOfBirth, state, city, email, new TestDefinerRole(),orga);
            orga.getTestDefinerDirectory().addTestDefiner(testDefiner);
            orga.getUserAccountDirectory().addUserAccount(testDefiner);
        }
        else{
            TestReporter testReporter = new TestReporter(name, contact, zipcode, city, gender, username, password, dateOfBirth, state, city, email, new TestReporterRole(),orga);
            orga.getTestReporterDirectory().addTestReporter(testReporter);
            orga.getUserAccountDirectory().addUserAccount(testReporter);
        }
        populateDoctorTable();
